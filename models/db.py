...
class DB:
    ...
    def __init__(self):
        ...
        host = getenv("HOST")
        self._engine = create_engine(f"postgresql+psycopg2://{user}:{password}@{host}:5432/logs")
        self._auth_engine = self._auth_engine = create_engine(f"postgresql+psycopg2://{user}:{password}@{host}:5432/auth")
